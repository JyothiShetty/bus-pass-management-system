...
try{
	...
	ResultSet r = s.executeQuery("select * from details where flag = 5 and user= \""+session.getAttribute("User")+"\"");
	while(r.next()){
		%>
		
		<form action="lastupdate.jsp" >
		    <input type="text" name="user" hidden value="<%=session.getAttribute("User")%>">
		    <h4>Hello Your Pass is renewaled</h4>
	...
	ResultSet r1 = s.executeQuery("select * from details where flag = 6 and user= \""+session.getAttribute("User")+"\"");
	while(r1.next()){
		%>
		
		<form action="lastupdate.jsp" >
		    <input type="text" name="user" hidden value="<%=session.getAttribute("User")%>">
		    <h4>Hello Your Pass is Rejected</h4>
		    <button type="submit" >OK</button>
		</form>
	...
	ResultSet r2 = s.executeQuery("select * from details where flag = 2 and user= \""+session.getAttribute("User")+"\"");
	while(r2.next()){
		%>
		
		<form action="lastupdate.jsp" >
		    <input type="text" name="user" hidden value="<%=session.getAttribute("User")%>">
		    <h4>Hello Your Pass is Accepted</h4>
		    <button type="submit" >OK</button>
		</form>
	...
	ResultSet r3 = s.executeQuery("select * from details where flag = 3 and user= \""+session.getAttribute("User")+"\"");
	while(r3.next()){
		%>
...
%> 
            ...
            <!-- <div class="carousel slide carousel-fade" data-ride="carousel"> -->
                        ...
                        <button class="btn btn-outline-info btn-lg text-white" herf="#">Student Access</button>
                    </div>
                </div>
            <!-- </div> -->
        </div>
